...
// true if expired or expires in < 10 seconds
CloudwatchBackend.prototype.credentialsNeedRefresh = function() {
  if (this.config.credentials && this.config.credentials.expireTime) {
    return (this.config.credentials.expireTime - new Date()) < 100;
  }

  return false;
};

CloudwatchBackend.prototype.refreshCredentials = function(cb) {
  if (this.config.credentials && this.config.credentials.refresh) {
    this.config.credentials.refresh(cb);
  }
};
...
CloudwatchBackend.prototype.sendBatch = function(data, namespace) {
  var self = this;
...
CloudwatchBackend.prototype.batchSend = function(currentMetricsBatch, namespace) {
  ...
  //DEBUG
  this.logger.log('credential: ' + util.inspect(this.config.credentials), 'DEBUG');
  this.logger.log(
    'credential expiration: ' + this.config.credentials.expireTime +
      ' (' + (this.config.credentials.expireTime - new Date()) + ')',
    'DEBUG'
  );
  //END DEBUG
  ...
  if (currentMetricsBatch.length > 0) {
    ...
    for (var i = 0, len = chunkedGroups.length; i < len; i++) {
      if (this.credentialsNeedRefresh()) {
        this.refreshCredentials(function(err) {
          if (err) {
            self.logger.log('Failed to refresh credentials: ' + util.inspect(err), 'ERROR');
          } else {
            self.logger.log('Credentials refreshed', 'INFO');
            self.sendBatch(chunkedGroups[0], namespace);
          }
        });
      } else {
        this.sendBatch(chunkedGroups[0], namespace);
      }
    }
  }
};
