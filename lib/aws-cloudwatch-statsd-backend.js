...
CloudwatchBackend.prototype.sendBatch = function(batchData, namespace) {
  var self = this;

  this.withCloudWatch(function(cloudwatch) {
    cloudwatch.putMetricData({
      MetricData: batchData,
      Namespace: namespace
    }, function(err, returnedData) {
      if (err) {
        // log an error
        self.logger.log('Error emitting metrics: ' + util.inspect(err), 'ERROR');
      } else {
        // Success
        self.logger.log('Emitted metrics: ' + util.inspect(returnedData), 'DEBUG');
      }
    });
  });
};

// construct a cloudwatch instance with valid credentials and pass it to the
// callback
CloudwatchBackend.prototype.withCloudWatch = function(cb) {
  if (this.config.credentials) {
    var self = this;
    console.log("DEBUG: creds are", this.config.credentials);
    this.config.credentials.get(function(err) {
      if (err) {
        self.logger.log('Error getting credentials: ' + util.inspect(err), 'ERROR');
      } else {
        cb(new AWS.CloudWatch(self.config));
      }
    });
  } else {
    cb(new AWS.CloudWatch(this.config));
  }
};
